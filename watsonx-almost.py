...
async def watsonx_stream() -> None:
    ...
    response = await llm.create(chat_model_input, stream=True)
    print(response.get_text_content())
...
async def watsonx_stream_abort() -> None:
    ...
    try:
        ...
        response = await llm.create(chat_model_input, stream=True, abort_signal=AbortSignal.timeout(0.5))
...
async def watson_structure() -> None:
    ...
    response = await llm.create_structure(TestSchema, chat_model_input) # Keep TestSchema as first positional arg, adjust chat_model_input to be second arg
...
async def watson_tool_calling() -> None:
    ...
    response = await watsonx_llm.create(chat_model_input, tools=[weather_tool]) # Adjust chat_model_input to be first arg
    ...
    final_response = await watsonx_llm.create(final_response_input, tools=[]) # Adjust final_response_input to be first arg
